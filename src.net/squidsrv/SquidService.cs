        ...
        private void OnUpdateTimer(object state)
        {
            lock (this.updateTimer)
            {
                try
                {
                    ...
                    var req = (HttpWebRequest)WebRequest.Create("https://www.diladele.com/squid/version/windows/version.json");
                    req.UserAgent = "Squid7.7/" + Environment.OSVersion.VersionString + "/x64 (win;0-0-0-0)";
